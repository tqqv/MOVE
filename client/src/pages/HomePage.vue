...
onMounted(() => {
  // Load những phần nhẹ và cần ngay
  fetchAllCategoriesHaveView();
  fetchVideoYouMayLike();

  // Featured (nặng nhất) defer để tránh lag
  if ('requestIdleCallback' in window) {
    requestIdleCallback(() => fetchDataSlider(currentDate.value));
  } else {
      ...
      <div class="items-center space-y-4">
        ...
        <div class="flex justify-between">
          ...
          <RouterLink to="/browse/categories">
            <span class="text-sm text-primary cursor-pointer">View all</span>
          </RouterLink>
        </div>
      </div>
      <div
